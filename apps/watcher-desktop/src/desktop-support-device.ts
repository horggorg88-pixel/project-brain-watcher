...
import {
  readDesktopAccessHandoff,
  readDesktopAccessHandoffToken,
} from "./desktop-access-handoff.js";
...
export async function enrollManagedDevice(
  ...
  const deviceToken =
    typeof parsed.deviceToken === "string" ? parsed.deviceToken : "";
  ...
  return {
    enrolled: true,
    status: toManagedDeviceStatus(state),
    message: "Support-устройство зарегистрировано.",
  };
}
...
export async function ensureManagedDeviceEnrolled(
  ...
  const existing = existingState
    ...
    : readManagedDeviceStatus(paths);
  if (
    existing.enrolled &&
    supportStateMatchesProject(existingState, projectId)
  ) {
    return {
      enrolled: true,
      status: existing,
  ...
  if (handoffAccount) {
    const enrollment = await enrollManagedDevice(
      paths,
      handoffAccount,
      projectId,
    );
  ...
  if (!storedAccount) {
    return {
      ...
      status: readManagedDeviceStatus(paths),
      message:
        "Нет сохранённого bearer для автоматической support-регистрации.",
    };
  }
  return enrollManagedDevice(
    paths,
    storedAccount.account,
    storedAccount.projectId,
  );
}
...
function accountFromHandoff(
  paths: DesktopCorePaths,
): DesktopAccountAuthorization | null {
  ...
  const consoleUrl = clientBaseUrl(
    handoff.consoleUrl,
...
function accountFromStoredProject(
  ...
  readonly account: DesktopAccountAuthorization;
  ...
  const serverUrl =
    normalizeMcpServerUrl(
      profile.serverUrl || config.serverUrl || DEFAULT_MCP_SERVER_URL,
    ) || DEFAULT_MCP_SERVER_URL;
  const consoleUrl = clientBaseUrl(
    profile.consoleUrl || config.consoleUrl || defaultConsoleUrlFor(serverUrl),
    DEFAULT_ACCOUNT_SERVER_URL,
  );
  return {
    ...
    account: {
      ...
      message:
        "Support enrollment восстановлен из сохранённого профиля проекта.",
    },
  };
}
...
function defaultConsoleUrlFor(serverUrl: string): string {
  return normalizeMcpServerUrl(serverUrl) === DEFAULT_MCP_SERVER_URL
    ? DEFAULT_ACCOUNT_SERVER_URL
    : "";
}
...
function clientBaseUrl(value: string, fallback: string): string {
  ...
  const normalizedFallback =
    normalizeMcpServerUrl(fallback) || DEFAULT_ACCOUNT_SERVER_URL;
...
function supportStateMatchesProject(
  state: SupportDeviceState | null,
...
function readSupportDeviceState(
  paths: DesktopCorePaths,
): SupportDeviceState | null {
  const path = statePath(paths);
  if (!existsSync(path)) return null;
  try {
    const parsed: unknown = JSON.parse(readFileSync(path, "utf-8"));
    return isSupportDeviceState(parsed) &&
      !isLocalBindUrl(parsed.supportBaseUrl)
      ? parsed
      : null;
  } catch {
    return null;
